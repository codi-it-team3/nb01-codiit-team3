...
export async function updateMyInfo(
  userId: string,
  data: { name?: string; password?: string; image?: string; currentPassword: string },
): Promise<User> {
  const user = await usersRepository.getUser(userId);
  if (!user) throw new NotFoundError('유저를 찾을 수 없습니다.', userId);

  const isPasswordValid = await verifyPassword(data.currentPassword, user.password);
  if (!isPasswordValid) throw new BadRequestError('현재 비밀번호가 일치하지 않습니다.');
